...
export default function ResponseCard() {
  ...
  const renderStep = () => {
    ...
    const avatarGradients = [
      "from-[#1A4D3E] to-[#2D7A5F]",
      "from-[#1A3D5C] to-[#2D6A8F]",
      "from-[#4A3D1E] to-[#8B7340]",
      "from-[#3D1A3D] to-[#6A2D6A]",
      "from-[#1A4D4D] to-[#2D7A7A]",
      "from-[#4D3A1A] to-[#9B7B3A]",
      "from-[#2A3A4D] to-[#4A6A8D]",
      "from-[#3D4D1A] to-[#6A8A2D]",
    ];
    const getGradient = (first: string, last: string) =>
      avatarGradients[(first.charCodeAt(0) + last.charCodeAt(0)) % avatarGradients.length];
        ...
        return (
          <div onKeyDown={handleKeyDown} className="relative">
            ...
            >
              {selectedAdvisor ? (
                <>
                  {(selectedAdvisor as any).photo_url ? (
                    <img src={(selectedAdvisor as any).photo_url} alt={`${selectedAdvisor.first_name} ${selectedAdvisor.last_name}`} className="w-10 h-10 rounded-full object-cover shrink-0" />
                  ) : (
            ...
            {selectedAdvisor && !agentOpen && (
              <motion.div
                ...
                className="mt-4 flex items-center gap-3 px-4 py-3 rounded-2xl bg-[#C8A96E]/10 border border-[#C8A96E]/30"
              >
                <div className={`w-10 h-10 rounded-full bg-gradient-to-br ${getGradient(selectedAdvisor.first_name, selectedAdvisor.last_name)} text-white flex items-center justify-center text-sm font-bold shrink-0 ring-2 ring-[#C8A96E]`}>
                  {selectedAdvisor.first_name[0]}{selectedAdvisor.last_name[0]}
                </div>
                <div>
