...
import { GraduationCap, Clock, Search, Plus, Pencil, Trash2 } from "lucide-react";
...
export default function AdminTraining() {
  ...
  async function loadData() {
    const { data } = await supabase.from("trainings").select("*").order("category");
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Manage Training</h1>
          <p className="text-sm text-gray-500 mt-0.5">Create and manage training content for all agents.</p>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        {filtered.map(t => (
          <div key={t.id} className="relative bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden flex flex-col h-full hover:shadow-md hover:border-gray-200 transition-all">
      ...
      {filtered.length === 0 && (<div className="bg-white rounded-xl border border-gray-100 shadow-sm text-center py-12"><GraduationCap className="h-12 w-12 text-gray-300 mx-auto mb-3" /><p className="text-gray-500">No trainings match your filters</p></div>)}
    </div>
  );
}
