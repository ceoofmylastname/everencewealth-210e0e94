...
import { Search, Star, Shield, Building2, X, Plus, Pencil, Trash2, ExternalLink } from "lucide-react";
...
export default function AdminCarriers() {
  ...
  async function loadCarriers() {
    ...
    setCarriers(data ?? []); setLoading(false);
  }
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {filtered.map(carrier => (
            <div key={carrier.id} className="bg-white rounded-xl border border-gray-100 shadow-sm p-5 flex flex-col hover:shadow-md hover:border-gray-200 transition-all h-full">
              ...
              {carrier.am_best_rating && (<span className={`text-xs w-fit mb-2 px-2.5 py-0.5 rounded-full font-medium ${getRatingBadge(carrier.am_best_rating)}`}>AM Best: {carrier.am_best_rating}</span>)}
              {carrier.products_offered && carrier.products_offered.length > 0 && (<div className="flex flex-wrap gap-1 mb-2">{carrier.products_offered.map((p: string) => (<span key={p} className="text-xs px-2 py-0.5 rounded-full bg-gray-100 text-gray-600 border border-gray-200">{p}</span>))}</div>)}
