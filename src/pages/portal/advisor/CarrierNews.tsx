...
function FilterChip({ label, active, onClick }: { label: string; active: boolean; onClick: () => void }) {
  return (
    <button onClick={onClick} className={`px-3 py-1 rounded-full text-xs font-medium transition-all cursor-pointer border ${
      active ? "text-white border-transparent" : "border-gray-200 text-gray-600 bg-white hover:bg-gray-50"
      ...
      {loading ? (
        ...
        <div className="space-y-3">
          {filteredNews.map(n => {
            ...
            return (
              <div key={n.id} className="bg-white rounded-xl border border-gray-100 shadow-sm p-5 cursor-pointer hover:shadow-md hover:border-gray-200 transition-all"
                onClick={() => incrementViews(n.id, n.views || 0)}>
                <div className="flex items-start gap-4">
                  {n.carriers?.carrier_logo_url && (
                    <img src={n.carriers.carrier_logo_url} alt={n.carriers.carrier_name} className="w-10 h-10 rounded object-contain bg-gray-50 p-0.5 flex-shrink-0" />
                  )}
                  <div className="flex-1 min-w-0">
                    <div className="flex items-start justify-between gap-3">
                      ...
                      <span className={`text-xs flex-shrink-0 px-2.5 py-0.5 rounded-full font-medium ${getPriorityBadge(n.priority)}`}>
                        {n.priority}
                      </span>
                    </div>
