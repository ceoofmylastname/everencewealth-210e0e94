...
import {
    ...
    X, Maximize, ExternalLink, ChevronDown, Layers, ArrowRight
} from 'lucide-react';

const ImageStudio = () => {
    const [activeTab, setActiveTab] = useState<'generate' | 'edit'>('generate');

    // Custom Comparison Slider State
                ...
                <div className="container mx-auto max-w-6xl relative z-10 flex flex-col lg:flex-row items-center gap-12">
                    ...
                    <div className="lg:w-1/2 flex flex-col items-start text-left">
                        ...
                        <motion.h1
                            ...
                            className="text-5xl md:text-6xl font-extrabold text-[#1A4D3E] leading-[1.1] tracking-tight mb-6"
                            style={syneFont}
                        >
                    ...
                    <div className="lg:w-1/2 w-full perspective-[1000px]">
                        <motion.div
                            animate={{ rotateY: [-8, 0, -8], rotateX: [4, 0, 4], y: [-5, 5, -5] }}
                            transition={{ duration: 8, repeat: Infinity, ease: "easeInOut" }}
                            className="relative w-full aspect-[4/3] rounded-2xl bg-white p-2 shadow-[0_24px_64px_rgba(26,77,62,0.14)] transform-gpu border border-gray-100"
                            style={{ transformStyle: 'preserve-3d' }}
                        >
                            <div className="w-full h-full rounded-xl overflow-hidden relative">
                                {/* Abstract CSS Art replacing real image */}
            ...
            <section className="sticky top-0 z-40 bg-white/80 backdrop-blur-xl border-y border-gray-100 py-4 shadow-sm mb-12">
                <div className="container mx-auto px-6 max-w-6xl flex justify-center">
                    <div className="bg-[#F7F9F8] p-1.5 rounded-full inline-flex relative shadow-inner border border-gray-200">
                        ...
                        <motion.div
                            className="absolute top-1.5 bottom-1.5 rounded-full bg-[#1A4D3E] shadow-sm pointer-events-none"
                            animate={{
                                left: activeTab === 'generate' ? '6px' : '50%',
                                width: activeTab === 'generate' ? 'calc(50% - 6px)' : 'calc(50% - 6px)'
                            }}
                        ...
                        <button
                            ...
                            className={`relative z-10 px-8 py-3 rounded-full text-sm font-bold tracking-wide transition-colors ${activeTab === 'generate' ? 'text-white' : 'text-gray-500 hover:text-gray-900'} flex items-center gap-2`}
                        >
                            <Wand2 size={16} /> Generate Image
                            {activeTab === 'generate' && <motion.div layoutId="underline" className="absolute -bottom-1 left-1/2 -translate-x-1/2 w-8 h-[2px] bg-[#C8A96E] rounded-full" />}
                        </button>

                        <button
                            onClick={() => setActiveTab('edit')}
                            className={`relative z-10 px-8 py-3 rounded-full text-sm font-bold tracking-wide transition-colors ${activeTab === 'edit' ? 'text-white' : 'text-gray-500 hover:text-gray-900'} flex items-center gap-2`}
                        >
                            <ImageIcon size={16} /> Edit Image
                            {activeTab === 'edit' && <motion.div layoutId="underline" className="absolute -bottom-1 left-1/2 -translate-x-1/2 w-8 h-[2px] bg-[#C8A96E] rounded-full" />}
                        </button>
                    </div>
                </div>
            </section>
                        ...
                        >
                            ...
                            <div className="lg:col-span-5 bg-white rounded-xl border-l-[4px] border-[#1A4D3E] border border-y-gray-200 border-r-gray-200 shadow-[0_4px_20px_rgba(0,0,0,0.03)] p-6 md:p-8 flex flex-col gap-8 focus-within:shadow-[0_8px_30px_rgba(200,169,110,0.15)] transition-shadow duration-300">
                                ...
                                <div>
                                    ...
                                    <textarea
                                        className="w-full min-h-[120px] bg-[#F7F9F8] border border-gray-200 rounded-lg p-4 text-gray-800 text-sm focus:outline-none focus:ring-2 focus:ring-[#C8A96E] focus:border-transparent transition-all resize-y placeholder:text-gray-400"
                                        placeholder="e.g. A confident financial advisor meeting with a family in a modern San Francisco office, warm lighting, professional atmosphere..."
                                    ></textarea>
                                </div>
                                ...
                                <div>
                                    ...
                                    <div className="flex overflow-x-auto pb-4 gap-2 scrollbar-none mask-fade-right">
                                        {['Professional Portrait', 'Corporate Scene', 'Abstract Financial', 'Lifestyle & Wealth', 'Clean Product Shot', 'Architectural'].map((preset, i) => (
                                            <button
                                                key={preset}
                                                className={`whitespace-nowrap px-4 py-2 rounded-full text-xs font-bold transition-all border ${i === 0 ? 'bg-[#1A4D3E] text-white border-[#1A4D3E]' : 'bg-white text-gray-600 border-gray-200 hover:border-[#1A4D3E]'}`}
                                            >
                                                {preset}
                                            </button>
                                        ))}
                                    ...
                                    <div className="grid grid-cols-2 gap-3">
                                        {[
                                            ...
                                            <button
                                                ...
                                                className={`flex items-center gap-3 p-3 rounded-lg border transition-all ${activeDimension === dim.id ? 'border-[#1A4D3E] bg-[#F7F9F8] shadow-sm' : 'border-gray-200 hover:border-gray-300 bg-white'}`}
                                            >
                                                <div className="w-10 h-10 flex items-center justify-center bg-white rounded border border-gray-100 flex-shrink-0">
                                                    <div className="bg-gray-300 rounded-sm" style={{ width: dim.w, height: dim.h, backgroundColor: activeDimension === dim.id ? '#1A4D3E' : '#D1D5DB' }}></div>
                                                </div>
                                ...
                                <div className="border border-gray-200 rounded-lg overflow-hidden">
                                    <button
                                        ...
                                        className="w-full bg-[#F7F9F8] p-4 flex items-center justify-between text-sm font-bold text-gray-700 hover:bg-gray-50 transition-colors"
                                    >
                                    ...
                                    <AnimatePresence>
                                        {isAdvancedOpen && (
                                            <motion.div
                                                ...
                                                className="p-4 bg-white border-t border-gray-200 space-y-4"
                                            >
                                                <div>
                                ...
                                {/* Generate Button */}
                                <button className="relative overflow-hidden w-full bg-[#1A4D3E] text-white py-4 rounded-lg font-bold text-lg shadow-[0_10px_30px_rgba(26,77,62,0.3)] hover:shadow-[0_15px_40px_rgba(26,77,62,0.4)] transition-all group mt-auto">
                            ...
                            <div className="lg:col-span-7 bg-[#F7F9F8] rounded-xl border border-[#1A4D3E]/10 p-6 md:p-8 flex flex-col justify-center min-h-[600px] relative overflow-hidden">
                                ...
                                <div className="w-full h-full flex flex-col items-center">
                                    ...
                                    {/* Generated Image CSS Placeholder */}
                                    <div className="w-full aspect-[4/3] rounded-xl overflow-hidden shadow-[0_20px_50px_rgba(0,0,0,0.1)] relative group mb-6 border border-gray-200">
                                        ...
                                        <div className="absolute top-4 right-4 bg-white/90 backdrop-blur-sm px-3 py-1 pb-1.5 pt-1.5 rounded-full shadow-lg border border-[#C8A96E]/30 flex items-center gap-1.5 text-[10px] uppercase font-black tracking-widest text-[#C8A96E]">
                                    ...
                                    <div className="flex flex-wrap items-center justify-center gap-3 w-full mb-8">
                                        <button className="flex-1 min-w-[120px] bg-[#1A4D3E] hover:bg-[#113328] text-white px-4 py-2.5 rounded-lg text-sm font-bold flex items-center justify-center gap-2 transition-colors shadow-sm">
                                            <Download size={16} /> Download
                                        </button>
                                        <button onClick={() => setActiveTab('edit')} className="flex-1 min-w-[120px] bg-white border-2 border-[#C8A96E] text-[#1A4D3E] hover:bg-[#C8A96E]/5 px-4 py-2.5 rounded-lg text-sm font-bold flex items-center justify-center gap-2 transition-colors">
                                            <Wand2 size={16} className="text-[#C8A96E]" /> Edit Image
                                        </button>
                                        <button className="flex-1 min-w-[120px] bg-white border border-[#1A4D3E] text-[#1A4D3E] hover:bg-[#F7F9F8] px-4 py-2.5 rounded-lg text-sm font-bold flex items-center justify-center gap-2 transition-colors">
                                            <RotateCcw size={16} /> Regenerate
                                        </button>
                                        <button className="px-4 py-2.5 rounded-lg text-gray-500 hover:text-gray-900 border border-transparent hover:bg-gray-100 transition-colors">
                                            <Copy size={20} />
                                        </button>
                                    </div>
                                    ...
                                    <div className="w-full">
                                        <h4 className="text-xs font-bold text-gray-500 uppercase tracking-widest mb-3 text-left">Variations</h4>
                                        <div className="grid grid-cols-3 gap-4">
                                            {[1, 2, 3].map((val, idx) => (
                                                <div
                                                    key={val}
                                                    onClick={() => setActiveVariation(idx)}
                                                    className={`aspect-[4/3] rounded-lg cursor-pointer overflow-hidden relative border-2 transition-all ${activeVariation === idx ? 'border-[#C8A96E] shadow-[0_0_15px_rgba(200,169,110,0.3)] scale-105 z-10' : 'border-transparent opacity-60 hover:opacity-100'}`}
                                                >
                                                    {/* Simulated variation gradients */}
                                                    <div className={`w-full h-full bg-gradient-to-br from-[#1A4D3E] to-[#0D1F1A] ${idx === 1 ? 'hue-rotate-15' : idx === 2 ? 'saturate-150' : ''}`}>
                    ...
                    {activeTab === 'edit' && (
                        ...
                        >
                            ...
                            <div className="lg:col-span-4 bg-white rounded-xl border border-gray-200 shadow-sm p-6 md:p-8 flex flex-col gap-8">
                                ...
                                {/* Upload Dropzone */}
                                <div className="w-full h-48 border-2 border-dashed border-[#1A4D3E]/40 rounded-xl bg-[#F7F9F8] hover:bg-[#1A4D3E]/5 flex flex-col items-center justify-center gap-3 cursor-pointer transition-colors relative group">
                                ...
                                <div>
                                    ...
                                    <textarea
                                        className="w-full min-h-[100px] bg-white border border-gray-200 rounded-lg p-4 text-gray-800 text-sm focus:outline-none focus:ring-2 focus:ring-[#1A4D3E] focus:border-transparent transition-all resize-y placeholder:text-gray-400 shadow-inner"
                                        placeholder="e.g. Make the background a modern San Francisco skyline, enhance the lighting, add a professional Evergreen color overlay..."
                                    ></textarea>
                                </div>
                                ...
                                <div className="space-y-3">
                                    {[
                                        ...
                                        <div
                                            ...
                                            className={`p-4 rounded-lg border-2 cursor-pointer transition-all flex items-start gap-4 ${editMode === mode.id ? 'border-[#1A4D3E] bg-[#1A4D3E]/5' : 'border-gray-100 hover:border-gray-200 bg-white'}`}
                                        >
                            ...
                            <div className="lg:col-span-8 bg-[#F7F9F8] rounded-xl border border-gray-200 p-6 md:p-8 flex flex-col justify-center min-h-[600px]">
                                ...
                                <div
                                    ref={sliderRef}
                                    className="relative w-full aspect-video md:aspect-[16/10] bg-gray-200 rounded-xl overflow-hidden shadow-xl select-none mx-auto border border-gray-300 touch-none"
                                    onPointerDown={handlePointerDown}
                                >
                                    {/* "After" Image (Background) - Represents output */}
                                    <div className="absolute inset-0 w-full h-full">
                                        {/* Placeholder gradient for After */}
                                        <div className="w-full h-full bg-[conic-gradient(from_90deg_at_50%_50%,#0D1F1A_0%,#1A4D3E_50%,#2D6B57_100%)]"></div>
                                    ...
                                    {/* "Before" Image (Foreground/Clipped) - Represents original */}
                                    <div
                                        className="absolute inset-y-0 left-0 overflow-hidden"
                                        style={{ width: `${sliderPosition}%` }}
                                    >
                                        {/* Original placeholder gradient */}
                                        <div className="w-screen max-w-[1000px] h-full bg-gradient-to-br from-gray-400 to-gray-600"></div>
                                    ...
                                    {/* Slider Handle Line */}
                                    <div
                                        className="absolute inset-y-0 w-1 bg-[#C8A96E] cursor-ew-resize flex items-center justify-center z-10 hover:w-1.5 transition-[width] shadow-[0_0_10px_rgba(200,169,110,0.5)]"
                                ...
                                <div className="text-center mt-6 text-xs text-gray-400 font-bold uppercase tracking-widest">
                                    Drag to Compare
                                </div>

                                {/* Quick Actions Row */}
                                <div className="flex flex-col sm:flex-row items-center justify-center gap-4 mt-8">
                                    <button className="w-full sm:w-auto px-6 py-3 rounded-lg border border-[#1A4D3E] text-[#1A4D3E] font-bold text-sm hover:bg-[#F7F9F8] transition-colors">
                                        Download Original
                                    </button>
                                    <button className="w-full sm:w-auto px-6 py-3 rounded-lg bg-[#1A4D3E] text-white font-bold text-sm hover:bg-[#113328] transition-colors shadow-md">
                                        Download Edited
                                    </button>
            ...
            <section className="py-24 container mx-auto px-6 max-w-6xl border-t border-gray-100 mt-24">
                <div className="flex justify-between items-end mb-12">
                    <div>
                        ...
                        <p className="text-gray-500 font-light text-lg">Your last 8 generated and edited images</p>
                    </div>
                    <button className="text-sm font-bold text-gray-400 hover:text-gray-800 transition-colors uppercase tracking-widest pb-1 border-b border-transparent hover:border-gray-800">
                        Clear History
                    </button>
                </div>
                ...
                <div className="columns-1 sm:columns-2 lg:columns-4 gap-6 space-y-6">
                    {[...Array(8)].map((_, i) => {
                        // Generate random but pleasant heights for masonry effect
                        const heights = ['h-64', 'h-48', 'h-80', 'h-56', 'h-72'];
                        const h = heights[i % heights.length];
                        // varied gradients
                        const gradients = [
                            'from-[#1A4D3E] to-[#0D1F1A]',
                            'from-slate-200 to-gray-300',
                            'from-[#C8A96E]/40 to-[#1A4D3E]/80',
                            'from-gray-800 to-gray-900'
                        ];
                        const grad = gradients[i % gradients.length];

                        return (
                            <div key={i} className={`w-full ${h} rounded-xl overflow-hidden relative group break-inside-avoid shadow-sm hover:shadow-2xl transition-all duration-500 border border-gray-200 bg-gradient-to-br ${grad}`}>
                                ...
                                <div className="absolute inset-0 bg-[#0D1F1A]/80 opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-center justify-center gap-3 border-2 border-transparent group-hover:border-[#C8A96E] rounded-xl box-border">
                                    <button className="w-10 h-10 rounded-full bg-white text-[#1A4D3E] flex items-center justify-center hover:scale-110 transition-transform">
                                        <Download size={18} />
                                    </button>
                                    <button className="w-10 h-10 rounded-full bg-[#C8A96E] text-white flex items-center justify-center hover:scale-110 transition-transform">
                                        <Wand2 size={18} />
                                    </button>
                                </div>
                            </div>
            ...
            <section className="py-24 bg-[#1A4D3E] relative overflow-hidden">
                ...
                <div className="container mx-auto px-6 max-w-5xl relative z-10 flex flex-col md:flex-row items-center justify-between gap-8 text-center md:text-left">
                    ...
                    <a
                        ...
                        className="shrink-0 bg-[#C8A96E] text-[#1A4D3E] px-8 py-4 rounded-full font-black text-lg tracking-wide hover:bg-white transition-colors shadow-[0_10px_30px_rgba(0,0,0,0.3)] flex items-center gap-2"
                    >
