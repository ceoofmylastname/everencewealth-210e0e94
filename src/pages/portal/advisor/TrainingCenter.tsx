...
function getLevelBadge(level: string) {
  switch (level) {
    ...
    case "intermediate": return "bg-blue-50 text-blue-700 border border-blue-200";
...
function FilterChip({ label, active, onClick }: { label: string; active: boolean; onClick: () => void }) {
  return (
    <button onClick={onClick} className={`px-3 py-1 rounded-full text-xs font-medium transition-all cursor-pointer capitalize border ${active ? "text-white border-transparent" : "border-gray-200 text-gray-600 bg-white hover:bg-gray-50"}`}
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-5 space-y-4">
        <div className="relative">
          ...
          <Input value={searchQuery} onChange={e => setSearchQuery(e.target.value)} placeholder="Search trainings..."
            className="pl-10 border-gray-200 bg-white text-gray-900 placeholder:text-gray-400 focus-visible:ring-1 rounded-lg" />
        </div>
        <div>
        ...
        <div>
          <p className="text-xs font-semibold text-gray-400 uppercase tracking-wide mb-2">Level</p>
          <div className="flex flex-wrap gap-2">
                  ...
                  {percent > 0 && !isCompleted && (
                    <div className="mb-3">
                      ...
                      <div className="h-1.5 rounded-full overflow-hidden bg-gray-100">
                        <div className="h-full rounded-full" style={{ width: `${percent}%`, background: BRAND_GREEN }} />
                      </div>
                    </div>
                  )}
      ...
      {filtered.length === 0 && (
        <div className="bg-white rounded-xl border border-gray-100 shadow-sm text-center py-12">
          ...
          <p className="text-gray-500">No trainings match your filters</p>
        </div>
      )}
    </div>
  );
