...
const CALC_COMPONENTS: Record<string, React.ComponentType<{ onClose: () => void }>> = {
  "IUL vs 401k Comparison": IULvs401k, "Inflation Impact Calculator": InflationImpact, "Retirement Gap Calculator": RetirementGap, "Social Security Estimator": SocialSecurityEstimator, "RMD Calculator": RMDCalculator, "Life Expectancy Calculator": LifeExpectancy, "Tax Bucket Optimizer": TaxBucketOptimizer, "Estate Tax Calculator": EstateTaxCalculator,
};
const CALC_TIME: Record<string, string> = { "IUL vs 401k Comparison": "~3 min", "Inflation Impact Calculator": "~1 min", "Retirement Gap Calculator": "~2 min", "Social Security Estimator": "~2 min", "RMD Calculator": "~2 min", "Life Expectancy Calculator": "~1 min", "Tax Bucket Optimizer": "~2 min", "Estate Tax Calculator": "~2 min" };
const CATEGORY_COLORS: Record<string, { bg: string; text: string; border: string }> = { cash_flow: { bg: "#EFF6FF", text: "#1D4ED8", border: "#BFDBFE" }, retirement: { bg: "#ECFDF5", text: "#065F46", border: "#6EE7B7" }, life_income: { bg: "#FDF4FF", text: "#7E22CE", border: "#E9D5FF" }, tax_planning: { bg: "#FFF7ED", text: "#C2410C", border: "#FED7AA" }, estate_planning: { bg: "#F5F3FF", text: "#6D28D9", border: "#DDD6FE" } };
...
export default function ToolsHub() {
  ...
  const handleOpenCalculator = (calcName: string) => { const component = CALC_COMPONENTS[calcName]; if (component) setOpenCalculator({ name: calcName, component }); };
    ...
    <div className="space-y-6">
        ...
        <DialogContent className="max-w-4xl w-full max-h-[90vh] overflow-y-auto p-0">
          <DialogHeader className="px-6 py-4 border-b border-gray-100 sticky top-0 bg-white z-10">
            <div className="flex items-center gap-3">
              <div className="h-8 w-8 rounded-lg flex items-center justify-center" style={{ background: `${BRAND_GREEN}15` }}><Calculator className="h-4 w-4" style={{ color: BRAND_GREEN }} /></div>
      ...
      <Tabs defaultValue="quoting">
        <TabsList className="bg-gray-100 rounded-lg p-1">
          <TabsTrigger value="quoting" className="rounded-md data-[state=active]:bg-white data-[state=active]:shadow-sm data-[state=active]:text-gray-900 text-gray-500"><Wrench className="h-4 w-4 mr-1" />Quoting Tools</TabsTrigger>
          <TabsTrigger value="calculators" className="rounded-md data-[state=active]:bg-white data-[state=active]:shadow-sm data-[state=active]:text-gray-900 text-gray-500"><Calculator className="h-4 w-4 mr-1" />Calculators</TabsTrigger>
        </TabsList>
              ...
              {filteredTools.map(t => (
                <div key={t.id} className="bg-white rounded-xl border border-gray-100 shadow-sm p-4 flex flex-col gap-3 hover:shadow-md hover:border-gray-200 transition-all">
                  ...
                  {t.login_instructions && (
                    <div>
                      <button onClick={() => setExpandedInstructions(expandedInstructions === t.id ? null : t.id)} className="text-xs flex items-center gap-1 hover:underline transition-opacity" style={{ color: BRAND_GREEN }}>
                        Login Instructions {expandedInstructions === t.id ? <ChevronUp className="h-3 w-3" /> : <ChevronDown className="h-3 w-3" />}
                      </button>
                      {expandedInstructions === t.id && (<p className="text-xs text-gray-500 mt-1 p-2 rounded-lg bg-gray-50 border border-gray-100">{t.login_instructions}</p>)}
                    </div>
                  )}
          ...
          {CALC_CATEGORIES.map(cat => {
            ...
            return (
              <div key={cat.key} className="space-y-3">
                <div className="flex items-center gap-2"><Icon className="h-4 w-4" style={{ color: BRAND_GREEN }} /><h3 className="text-sm font-semibold text-gray-700 uppercase tracking-wider">{cat.label}</h3><span className="text-xs px-1.5 rounded-full text-gray-400 bg-gray-100">{categoryCalcs.length}</span></div>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  {categoryCalcs.map(c => {
                    const CatIcon = cat.icon; const hasComponent = !!CALC_COMPONENTS[c.calculator_name]; const timeLabel = CALC_TIME[c.calculator_name] || "~2 min";
                    return (
                      <div key={c.id} className="bg-white rounded-xl border border-gray-100 shadow-sm p-4 flex flex-col gap-3 hover:shadow-md hover:border-gray-200 transition-all">
                        <div className="flex items-start gap-3">
                          <div className="h-9 w-9 rounded-lg flex items-center justify-center shrink-0" style={{ background: `${BRAND_GREEN}15` }}><CatIcon className="h-5 w-5" style={{ color: BRAND_GREEN }} /></div>
                          <div className="min-w-0 flex-1">
                            <div className="flex items-start justify-between gap-2">
                              <p className="font-medium text-gray-900">{c.calculator_name}</p>
                              <span className="text-xs px-2 py-0.5 rounded-full border font-medium shrink-0" style={{ background: colors.bg, color: colors.text, borderColor: colors.border }}>{cat.label}</span>
                            </div>
                            {c.description && <p className="text-xs text-gray-400 mt-1 line-clamp-2">{c.description}</p>}
                          </div>
                        </div>
                        <div className="flex items-center justify-between">
                          <span className="text-xs text-gray-400 flex items-center gap-1"><span className="w-1.5 h-1.5 rounded-full inline-block" style={{ background: BRAND_GREEN }} />{timeLabel}</span>
                          {hasComponent && (<span className="text-xs px-2 py-0.5 rounded-full font-medium text-white" style={{ background: BRAND_GREEN }}>Interactive</span>)}
                        </div>
                        <button onClick={() => handleOpenCalculator(c.calculator_name)} disabled={!hasComponent} className="w-full py-2 rounded-lg text-xs font-semibold flex items-center justify-center gap-1.5 text-white transition-all hover:opacity-90 disabled:opacity-40 disabled:cursor-not-allowed" style={{ background: BRAND_GREEN }}><Calculator className="h-3.5 w-3.5" />Open Calculator</button>
                      </div>
                    );
                  })}
                </div>
