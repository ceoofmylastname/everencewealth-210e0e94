      ...
      {done ? (
        <div className="bg-white border rounded-lg p-8 text-center space-y-3">
          ...
          <div className="flex justify-center gap-2 pt-2">
            <Button variant="outline" onClick={() => { setDone(null); setRows([]); setHeaders([]); setMapping({}); }}>
              Import another file
            </Button>
            <Button onClick={() => navigate("/portal/advisor/contacts")}>View contacts</Button>
          </div>
        </div>
      ) : rows.length === 0 ? (
        ...
        <>
          <div className="bg-white border rounded-lg overflow-hidden">
            ...
            <div className="divide-y">
              {headers.map((h) => (
                <div key={h} className="px-5 py-3 grid grid-cols-2 gap-3 items-center">
                  <div>
          ...
          <div className="flex justify-end gap-2">
            <Button variant="outline" onClick={() => { setRows([]); setHeaders([]); setMapping({}); }}>
              Cancel
            </Button>
