            ...
            <div className="relative backdrop-blur-xl bg-white/90 border border-white/60 shadow-[0_8px_32px_rgba(0,0,0,0.12),0_0_0_1px_rgba(255,255,255,0.5)_inset] rounded-2xl p-6 md:p-8 space-y-5 hover:shadow-[0_20px_60px_hsl(var(--prime-gold)/0.12)] transition-all duration-500">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                <div className="space-y-2">
                  ...
                  <Input placeholder={ps.placeholders.reference} value={reference} onChange={(e) => setReference(e.target.value)} className={inputStyles} />
                </div>
                <div className="space-y-2">
                  <label className={labelStyles}>{ps.labels.location}</label>
                  <Select value={location} onValueChange={setLocation}>
                    <SelectTrigger className={selectTriggerStyles}>
                      {locationsLoading ? <span className="flex items-center gap-2 text-muted-foreground"><Loader2 className="h-4 w-4 animate-spin" />{ps.placeholders.loading}</span> : <SelectValue placeholder={ps.placeholders.anyLocation} />}
                    </SelectTrigger>
                ...
                <div className="space-y-2">
                  ...
                  <Select value={propertyType} onValueChange={setPropertyType}>
                    <SelectTrigger className={selectTriggerStyles}>
                      {typesLoading ? <span className="flex items-center gap-2 text-muted-foreground"><Loader2 className="h-4 w-4 animate-spin" />{ps.placeholders.loading}</span> : <SelectValue placeholder={ps.placeholders.anyType} />}
                    </SelectTrigger>
                    <SelectContent className="bg-white/95 backdrop-blur-xl border-white/60 shadow-xl z-50 max-h-80 rounded-xl">
                      ...
                      {flattenedTypes.map((type) => <SelectItem key={type.value} value={type.value}>{type.label}</SelectItem>)}
                    </SelectContent>
                  </Select>
                </div>
                <div className="space-y-2">
                  <label className={`${labelStyles} invisible`}>{ps.buttons.search}</label>
                  <Button onClick={handleSearch} disabled={isLoading} className="w-full h-12 bg-gradient-to-r from-prime-gold to-prime-goldLight text-prime-900 font-semibold rounded-xl shadow-lg shadow-prime-gold/30 hover:shadow-xl hover:shadow-prime-gold/40 hover:scale-[1.02] active:scale-[0.98] transition-all duration-300 border-0">
                    <Search className="w-5 h-5 mr-2" />{ps.buttons.search}
                  </Button>
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-4">
                <div className="space-y-2">
                  ...
                  <Select value={bedrooms} onValueChange={setBedrooms}>
                    <SelectTrigger className={selectTriggerStyles}><SelectValue placeholder={ps.placeholders.any} /></SelectTrigger>
                    <SelectContent className="bg-white/95 backdrop-blur-xl border-white/60 shadow-xl z-50 rounded-xl">
                      {BEDROOM_OPTIONS.map((opt) => <SelectItem key={opt.value || "any"} value={opt.value || "any"}>{opt.value ? opt.label : ps.placeholders.any}</SelectItem>)}
                    </SelectContent>
                  </Select>
                </div>
                <div className="space-y-2">
                  ...
                  <Select value={priceMin} onValueChange={setPriceMin}>
                    <SelectTrigger className={selectTriggerStyles}><SelectValue placeholder={ps.placeholders.any} /></SelectTrigger>
                    <SelectContent className="bg-white/95 backdrop-blur-xl border-white/60 shadow-xl z-50 rounded-xl">
                      {PRICE_OPTIONS.map((opt) => <SelectItem key={opt.value || "any-min"} value={opt.value || "any"}>{opt.value ? opt.label : ps.placeholders.any}</SelectItem>)}
                    </SelectContent>
                  </Select>
                </div>
                <div className="space-y-2">
                  ...
                  <Select value={priceMax} onValueChange={setPriceMax}>
                    <SelectTrigger className={selectTriggerStyles}><SelectValue placeholder={ps.placeholders.any} /></SelectTrigger>
                    <SelectContent className="bg-white/95 backdrop-blur-xl border-white/60 shadow-xl z-50 rounded-xl">
                      {PRICE_OPTIONS.map((opt) => <SelectItem key={opt.value || "any-max"} value={opt.value || "any"}>{opt.value ? opt.label : ps.placeholders.any}</SelectItem>)}
                    </SelectContent>
                  </Select>
                </div>
                <div className="space-y-2">
                  ...
                  <Select value={status} onValueChange={setStatus}>
                    <SelectTrigger className={selectTriggerStyles}><SelectValue placeholder={ps.status.newDevelopments} /></SelectTrigger>
                    <SelectContent className="bg-white/95 backdrop-blur-xl border-white/60 shadow-xl z-50 rounded-xl">
                ...
                <div className="space-y-2">
                  ...
                  <Button variant="ghost" onClick={handleReset} className="w-full h-12 text-slate-500 hover:text-prime-gold hover:bg-prime-gold/10 rounded-xl border border-slate-200/50 hover:border-prime-gold/30 transition-all duration-300">
                    <RotateCcw className="w-4 h-4 mr-2" />{ps.buttons.clearAll}
                  </Button>
                </div>
              </div>
            </div>
