...
const FloatingIcosahedron = () => (
  <motion.div
    ...
    style={{
      ...
      boxShadow: '0 0 60px hsla(43,74%,49%,0.3)',
    }}
  />
);
...
export const IULHero: React.FC = () => {
  ...
  return (
    <section className="relative min-h-[90vh] flex items-center overflow-hidden shadow-[0_20px_60px_-15px_rgba(0,0,0,0.5)]" style={{ background: 'linear-gradient(135deg, hsl(160,48%,14%) 0%, hsl(160,48%,8%) 60%, hsl(160,48%,5%) 100%)' }}>
      ...
      <div className="absolute right-[5%] top-1/2 -translate-y-1/2 w-[300px] h-[300px] md:w-[400px] md:h-[400px] opacity-60 pointer-events-none hidden lg:block flex items-center justify-center">
        <FloatingIcosahedron />
      </div>
      ...
      <div className="container max-w-6xl mx-auto px-6 py-24 text-center relative z-10">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 1.1 }} className="flex flex-wrap gap-4 justify-center">
          <motion.a href="#iul-cta" whileHover={{ scale: 1.05, boxShadow: '0 0 30px hsla(43,74%,49%,0.4)' }} whileTap={{ scale: 0.98 }} className="px-8 py-4 rounded-xl font-bold text-lg transition-all duration-300" style={{ background: 'linear-gradient(135deg, hsl(43,74%,49%) 0%, hsl(43,74%,55%) 100%)', color: 'hsl(160,48%,12%)' }}>
            {s?.ctaPrimary || 'Get Personalized Illustration'}
          </motion.a>
          <motion.a href="#iul-comparison" whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.98 }} className="px-8 py-4 rounded-xl font-bold text-lg border border-white/30 text-white hover:bg-white/10 transition-all duration-300">
