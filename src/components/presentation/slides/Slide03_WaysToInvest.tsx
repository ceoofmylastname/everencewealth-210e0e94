...
export default function Slide03_WaysToInvest() {
  return (
    <div className="antigravity-slide">
      ...
      <MorphBlob size={300} color="rgba(26, 77, 62, 0.08)" top="-5%" left="-4%" delay={0} />
      <MorphBlob size={280} color="rgba(200, 169, 110, 0.10)" bottom="-8%" right="-6%" delay={5} />
          ...
          {columns.map((col, i) => (
            <RevealElement key={col.title} index={i + 2} direction="cardRise">
              ...
              >
                ...
                />
                <h3 className="text-2xl font-bold mb-2" style={{ color: "var(--ev-green)" }}>
                  {col.title}
                </h3>
                <p className="text-sm" style={{ color: "var(--ev-text-light)" }}>{col.items}</p>
                {col.recommended && (
                  <div
