  ...
  return (
    <div className="space-y-3">
      <div className="flex justify-end">
        <Button size="sm" onClick={() => setAdding(!adding)} style={{ backgroundColor: "#1A4D3E" }}><Plus className="w-4 h-4 mr-1" /> Add Reminder</Button>
      </div>
      {adding && (
        <div className="bg-white border rounded-lg p-4 space-y-3">
          ...
          <div className="flex justify-end gap-2">
            ...
            <Button size="sm" onClick={save} style={{ backgroundColor: "#1A4D3E" }}>Set Reminder</Button>
          </div>
        </div>
      )}
      {items.length === 0 && !adding ? (
        ...
        return (
          <div key={r.id} className={`bg-white border rounded-lg p-4 flex justify-between items-start ${overdue ? "border-red-300 bg-red-50" : ""} ${done ? "opacity-50" : ""}`}>
            <div className="flex gap-3">
              <Bell className={`w-5 h-5 mt-0.5 ${overdue ? "text-red-600" : "text-gray-500"}`} />
              <div>
            ...
            {!done && (
              <div className="flex gap-1">
                <Button variant="ghost" size="sm" onClick={() => complete(r.id)}><Check className="w-4 h-4 text-emerald-600" /></Button>
                <Button variant="ghost" size="sm" onClick={() => dismiss(r.id)}><X className="w-4 h-4 text-gray-500" /></Button>
              </div>
            )}
          </div>
        );
