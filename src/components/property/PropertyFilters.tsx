        ...
        <div className="p-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <div className="space-y-2">
              <label className="text-xs font-semibold text-muted-foreground uppercase tracking-wider">{t.filters.location}</label>
              <Select value={location} onValueChange={(val) => { setLocation(val); setSublocation(""); }}>
                ...
                <SelectContent className="bg-white/95 backdrop-blur-xl border-border/50 rounded-xl shadow-xl">
                  <SelectItem value="any" className="rounded-lg">{t.filters.anyLocation}</SelectItem>
                  {locations.map((loc) => (
            ...
            {/* Property Type */}
            <div className="space-y-2">
              <label className="text-xs font-semibold text-muted-foreground uppercase tracking-wider">{t.filters.propertyType}</label>
              <Select value={propertyType} onValueChange={setPropertyType}>
                ...
                <SelectContent className="bg-white/95 backdrop-blur-xl border-border/50 rounded-xl shadow-xl max-h-80">
                  <SelectItem value="any" className="rounded-lg">{t.filters.anyType}</SelectItem>
            ...
            {/* Price Range */}
            <div className="space-y-2">
              <label className="text-xs font-semibold text-muted-foreground uppercase tracking-wider">{t.filters.priceRange}</label>
              <div className="grid grid-cols-2 gap-2">
                <Select value={priceMin} onValueChange={setPriceMin}>
                  <SelectTrigger className="h-12 bg-white/60 border-border/50 rounded-xl hover:border-primary/50 focus:border-primary focus:ring-2 focus:ring-primary/20 transition-all duration-300">
                    <SelectValue placeholder={t.filters.min} />
                  </SelectTrigger>
                  <SelectContent className="bg-white/95 backdrop-blur-xl border-border/50 rounded-xl shadow-xl">
                    {PRICE_OPTIONS.map((opt) => (
                      <SelectItem key={opt.value || "any-min"} value={opt.value || "any"} className="rounded-lg">
                        {opt.value === "" ? t.filters.any : opt.label}
                      </SelectItem>
                    ))}
                  </SelectContent>
                </Select>
                <Select value={priceMax} onValueChange={setPriceMax}>
                  <SelectTrigger className="h-12 bg-white/60 border-border/50 rounded-xl hover:border-primary/50 focus:border-primary focus:ring-2 focus:ring-primary/20 transition-all duration-300">
                    <SelectValue placeholder={t.filters.max} />
                  </SelectTrigger>
                  <SelectContent className="bg-white/95 backdrop-blur-xl border-border/50 rounded-xl shadow-xl">
                    {PRICE_OPTIONS.map((opt) => (
            ...
            {/* Search Button */}
            <div className="space-y-2">
              <label className="text-xs font-semibold text-muted-foreground uppercase tracking-wider invisible">{t.filters.search}</label>
              <Button
              ...
              >
                ...
                {t.filters.search}{resultCount !== undefined && resultCount > 0 && ` (${resultCount.toLocaleString()})`}
              </Button>
            </div>
          </div>
              ...
              <button className="flex items-center gap-2 mt-4 text-sm font-medium text-muted-foreground hover:text-primary transition-colors duration-300 group">
                ...
                {activeFilters > 0 && (
                  <span className="px-2 py-0.5 bg-primary/10 text-primary text-xs font-semibold rounded-full">
                    {activeFilters} {t.filters.active}
                  </span>
                )}
            ...
            <CollapsibleContent className="overflow-hidden data-[state=open]:animate-accordion-down data-[state=closed]:animate-accordion-up">
              <div className="pt-4 mt-4 border-t border-border/30">
                <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-4">
                  {/* Reference */}
                  <div className="space-y-2">
                    <label className="text-xs font-semibold text-muted-foreground uppercase tracking-wider">{t.filters.reference}</label>
                    <Input
                  ...
                  {/* Sublocation */}
                  <div className="space-y-2">
                    <label className="text-xs font-semibold text-muted-foreground uppercase tracking-wider">{t.filters.sublocation}</label>
                    <Select value={sublocation} onValueChange={setSublocation} disabled={!location || location === "any"}>
                      ...
                      <SelectContent className="bg-white/95 backdrop-blur-xl border-border/50 rounded-xl shadow-xl">
                        <SelectItem value="any" className="rounded-lg">{t.filters.anySublocation}</SelectItem>
                      </SelectContent>
                    </Select>
                  </div>
                  ...
                  {/* Bedrooms */}
                  <div className="space-y-2">
                    <label className="text-xs font-semibold text-muted-foreground uppercase tracking-wider">{t.filters.bedrooms}</label>
                  ...
                  {/* Bathrooms */}
                  <div className="space-y-2">
                    <label className="text-xs font-semibold text-muted-foreground uppercase tracking-wider">{t.filters.bathrooms}</label>
                  ...
                  {/* Status */}
                  <div className="space-y-2">
                    <label className="text-xs font-semibold text-muted-foreground uppercase tracking-wider">{t.filters.status}</label>
                    <Select value={status} onValueChange={setStatus}>
                      <SelectTrigger className="h-11 bg-white/60 border-border/50 rounded-xl hover:border-primary/50 transition-all duration-300">
                        <SelectValue placeholder={t.filters.sales} />
                      </SelectTrigger>
                      <SelectContent className="bg-white/95 backdrop-blur-xl border-border/50 rounded-xl shadow-xl">
                  ...
                  {/* Reset Button */}
                  <div className="space-y-2">
                    <label className="text-xs font-semibold text-muted-foreground uppercase tracking-wider invisible">{t.filters.reset}</label>
                    <Button
                      variant="outline"
                      onClick={handleReset}
