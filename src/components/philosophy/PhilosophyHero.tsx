...
export const PhilosophyHero: React.FC = () => {
  ...
  const stats = (h as any).stats as Array<{ value: string; label: string }> | undefined;
  ...
  return (
    ...
    >
      ...
      {/* Morphing blobs with parallax (reduced to 2) */}
      ...
      <div className="relative z-10 max-w-5xl mx-auto px-4 md:px-8 text-center">
        {/* Gold divider line — draws in from center */}
        <motion.div
          initial={{ scaleX: 0 }}
          animate={{ scaleX: 1 }}
        ...
        <motion.p
          ...
          className="text-xs font-space font-bold tracking-[0.3em] uppercase text-white/40 mb-8"
        >
          {h.badge}
        </motion.p>

        {/* Headline — blur-to-sharp */}
        <motion.h1
          ...
          >
            {stats.map((stat, i) => (
              <React.Fragment key={i}>
                {i > 0 && (
                  <div className="w-px h-10 bg-white/10 mx-6 md:mx-10" />
                )}
                <motion.div
                ...
                >
                  <span className="block text-2xl md:text-3xl font-space font-bold text-[hsl(43,74%,49%)]">
                    {stat.value}
                  </span>
                  <span className="block text-xs md:text-sm text-white/40 font-space tracking-wider uppercase mt-1">
                    {stat.label}
                  </span>
                </motion.div>
              </React.Fragment>
