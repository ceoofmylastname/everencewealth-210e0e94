...
Deno.serve(async (req) => {
  ...
  try {
    ...
    await supabase.from('article_revisions').insert({
      ...
      revision_type: 'code_fence_rollback',
      changed_by: user_id,
      change_reason: `Rolled back code fence cleanup (revision ${revision_id})`,
      can_rollback: false,
    });
    ...
    await supabase.from('content_updates').insert({
      article_id: revision.article_id,
      update_type: 'code_fence_rollback',
      updated_fields: ['detailed_content'],
      update_notes: `Rolled back code fence cleanup to previous state`,
      updated_by: user_id,
